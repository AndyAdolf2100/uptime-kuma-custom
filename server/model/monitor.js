...
class Monitor extends BeanModel {
    ...
    async toJSON(includeSensitiveData = true) {
        ...
        let data = {
            ...
            jsonResultForm: this.jsonResultForm,
            screenshot,
        };
                ...
                if (await Monitor.isUnderMaintenance(this.id)) {
                    ...
                    if (this.getUrl()?.protocol === "https:") {
                        ...
                        try {
                            ...
                            if (
                                !this.getIgnoreTls() &&
                                this.isEnabledExpiryNotification()
                            ) {
                                log.debug(
                                    "monitor",
                                    `[${this.name}] call checkCertExpiryNotifications`
                                );
                                await this.checkCertExpiryNotifications(
                                    tlsInfoObject
                                );
                            }
                        } catch (e) {
                            if (
                                e.message !== "No TLS certificate in response"
                            ) {
                    ...
                    if (this.type === "http") {
                    ...
                    } else if (this.type === "json-result-form") {
                        log.debug("json-result-form start");
                        let data = res.data;
                        const isObject = (data) => {
                        ...
                        const getDataConstruction = (data, str = "") => {
                            const judger = isObject(data);
                            if (judger) {
                                if (judger === "array") {
                                    str += "[";
                                } else {
                                ...
                                keys.forEach((key, index) => {
                                    ...
                                    if (
                                        index === 0 ||
                                        (index > 0 && judger === "object")
                                    ) {
                                        if (index > 0) {
                                            str += ",";
                                        }
                                        str += getDataConstruction(
                                            content,
                                            judger === "array" ? "" : key
                                        );
                                    }
                        ...
                        log.debug(
                            "json-result-form end",
                            getDataConstruction(data)
                        );
                        if (
                            this.json_result_form === getDataConstruction(data)
                        ) {
                            bean.msg += " JSON result form is correct";
                            bean.status = UP;
                        } else {
                            throw new Error("JSON result form has been changed");
                        }
                    }
